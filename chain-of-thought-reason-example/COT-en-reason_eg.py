...
This script evaluates questions using a simple reasoning prompt that requests an example (English version).
...
def save_results_to_latex(all_model_results, output_path):
    ...
    latex_content.append("\\title{Chain of Thought Reasoning with Example Evaluation Results}")
...
def main():
    ...
    print("=" * 80)
    print("Chain of Thought (COT) Evaluation - Simple Reasoning with Example")
    ...
    txt_output_path = os.path.join(script_dir, "COT_en_reason_eg_summary.txt")
    csv_output_path = os.path.join(script_dir, "COT_en_reason_eg_results.csv")
    latex_output_path = os.path.join(script_dir, "COT_en_reason_eg_results.tex")
