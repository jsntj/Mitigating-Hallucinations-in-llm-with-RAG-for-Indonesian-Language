...
def format_prompt(question, choices):
    """Format the question and choices into a prompt for the model with a reasoning example"""
    
    # Example showing how to reason through a question step by step
    example = """Example of how to think step by step:

Question: Membaca buku sejarah, melihat berbagai peninggalan sejarah yang unik dan menarik dapat menggugah rasa estetika, kekaguman, dan keanehan terhadap kemampuan manusia masa lampau beserta peradabannya. Sehubungan dengan itu sejarah memiliki nilai....
A. edukatif
B. inspiratif
C. rekreatif
D. manipulatif
...
Reasoning:
1. Understanding the question: The question asks about what value history has when reading history books and seeing historical artifacts that evoke aesthetic feelings, admiration, and wonder about past human capabilities and civilizations.

2. Analyzing key concepts:
   - The question mentions: aesthetic feelings, admiration, wonder
   - These are emotional and experiential responses
   - The focus is on the experience of engaging with history

3. Evaluating each option:
   - Option A (edukatif/educational): This is about learning, but the question emphasizes emotional/experiential aspects, not just learning.
   - Option B (inspiratif/inspirational): This could work, but the question specifically mentions "recreation" and aesthetic enjoyment.
   - Option C (rekreatif/recreational): This matches perfectly - the question describes an enjoyable, aesthetic experience that provides pleasure and wonder, which is recreational in nature.
   - Option D (manipulatif/manipulative): This is negative and doesn't fit the positive description.
   - Option E (persuasif/persuasive): This is about convincing, not about the experience described.

4. Conclusion: The description emphasizes the recreational, enjoyable aspect of engaging with history through aesthetic appreciation and wonder. This aligns with "rekreatif" (recreational value).
...
Now, apply the same step-by-step reasoning process to the question above:
1. First, carefully read and understand what the question is asking
2. Identify key concepts, terms, and context in the question
3. Evaluate each option systematically:
   - Consider what each option means
   - Check how well it matches the question's requirements
   - Look for clues in the question text
4. Use logical reasoning to eliminate incorrect options
5. Choose the single best answer based on your analysis

After completing your reasoning, clearly separate it from the final answer.
Write your step-by-step reasoning in full sentences.
On the very last line, provide only the final answer letter (A, B, C, D, or E). Answer:"""
...
def main():
    ...
    # For debugging / initial CoT analysis, use the smaller sampled file
    csv_path = os.path.join(script_dir, "IndoMMLU_samples_short.csv")
    ...
    # Save detailed CoT results to CSV (question, ground truth, prediction, reasoning)
    print("\nSaving detailed Chain-of-Thought results to CSV...")
    csv_rows = []
    for model_result in all_model_results:
        if 'error' in model_result:
            continue
    ...
    if csv_rows:
        output_csv_path = os.path.join(script_dir, "COT-en-2_results.csv")
        df_out = pd.DataFrame(csv_rows)
        df_out.to_csv(output_csv_path, index=False)
        print(f"✓ Saved detailed CoT results to CSV: {output_csv_path}")
    else:
        print("⚠ No results available to save to CSV.")
