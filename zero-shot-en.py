"""
Zero-shot evaluation script for IndoMMLU dataset (multi-model).
This script evaluates questions from the IndoMMLU_samples.csv file using multiple models.
Model names are loaded from seallm.txt (same as COT-en.py).
"""
...
def evaluate_zero_shot(model, tokenizer, questions, model_name=""):
    """Evaluate questions using zero-shot approach"""
    results = []
    ...
    if len(questions) > 0:
        ...
        print(f"\n  Timing stats: {len(questions)} questions in {total_time:.2f}s")
...
def load_and_evaluate_model(model_name, groups, csv_path, num_questions_per_group):
    """
    Load a model and evaluate it on all groups using zero-shot prompting.
    ...
    """
    print(f"\n{'=' * 80}")
    print(f"Loading and evaluating model (zero-shot): {model_name}")
    print(f"{'=' * 80}")
    ...
    try:
        ...
        all_results = []
        print("\n" + "=" * 80)
        print(f"Evaluating questions with {model_name} (zero-shot)")
        print("=" * 80)
...
def main():
    """Main function to run zero-shot evaluation on multiple models"""
    print("=" * 80)
    print("Zero-Shot Evaluation - Multi-Model Comparison")
    ...
    print("\n\n" + "=" * 80)
    print("COMPARATIVE SUMMARY - ALL MODELS (ZERO-SHOT)")
    print("=" * 80)
    ...
    if valid_results:
        ...
        print(f"\n🏆 Best Model (Zero-Shot): {best_model['model_name']} with {best_model['accuracy']:.2f}% accuracy")
    else:
        print(f"\n⚠ No valid results to determine best model")
    
    # Print detailed statistics by subject for each model
    print("\n\n" + "=" * 80)
    print("DETAILED STATISTICS BY MODEL (ZERO-SHOT)")
    print("=" * 80)
